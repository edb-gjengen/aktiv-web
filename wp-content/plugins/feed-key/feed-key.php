<?php
/* 
Plugin Name: Feed Key
Plugin URI:  http://code.andrewhamilton.net/wordpress/plugins/feed-key/
Description: Feed Key adds a 32bit (or 40bit) key for each of your users, creating a unique feed url for every registered on user the site. This allows you to restrict you feeds to registered users only.
Version: 0.2
...
register_activation_hook(__FILE__,'feedkey_setup_options');
...
{
	global $userdata, $feedkey_valid, $errormsg, $feedkey_opt, $wpdb;
	...
	if (!empty($submitted_feedkey))
	{
		...
		$find_feedkey = $wpdb->get_results("SELECT umeta_id FROM wp_usermeta WHERE meta_value = '$submitted_feedkey'");
...
add_action('admin_menu', 'feedkey_add_options_page');
add_action('template_redirect', 'feedkey');
